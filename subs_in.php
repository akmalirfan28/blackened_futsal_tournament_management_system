 ...
 <h2>Substitute In</h2><br>
    <table>
      ...
      <?php
        ...
        $records_team = mysqli_query($conn, "SELECT playerID, name FROM player WHERE teamID = '$team_id' AND playerID != '$player_out_id'");
        
        while($data = mysqli_fetch_array($records_team))
        {
          ?>
            <tr>
                <td><?php echo $data['name'];?></td>
                <td><a href="substitution.php?match_id=<?php echo $matchID;?>&player_out_id=<?php echo $player_out_id;?>&team_id=<?php echo $team_id;?>&player_in_id=<?php echo $data['playerID'];?>"><button class="btn btn-sm">In</button></a></td>
            </tr>
          <?php
        }
        ?>
