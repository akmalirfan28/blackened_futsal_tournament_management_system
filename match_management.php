...
$stmt_update_score = $conn->prepare("UPDATE match_schedule SET match_status = 'Ongoing' WHERE match_id = ?");
$stmt_update_score->bind_param("i", $_SESSION['matchID']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'teamA_minus':
                if ($_SESSION['teamA_score'] > 0) {
                    $_SESSION['teamA_score']--;
                    // Update database
                    $stmt_update_score = $conn->prepare("UPDATE match_schedule SET team1_score = ? WHERE match_id = ?");
                    $stmt_update_score->bind_param("ii", $_SESSION['teamA_score'], $matchID);
                    $stmt_update_score->execute();
                    $stmt_update_score->close();
                }
                break;
            case 'teamB_minus':
                if ($_SESSION['teamB_score'] > 0) {
                    $_SESSION['teamB_score']--;
                    // Update database
                    $stmt_update_score = $conn->prepare("UPDATE match_schedule SET team2_score = ? WHERE match_id = ?");
                    $stmt_update_score->bind_param("ii", $_SESSION['teamB_score'], $matchID);
                    $stmt_update_score->execute();
                    $stmt_update_score->close();
                }
                break;
            case 'end_match':
                header('location:end_match.php');    
                break;
        }
    }
}
        ...
        <div class="score-box">
          ...
          ?>
            <div>
                ...
                <a href="goalscorerA.php?NAME=<?php echo $data['teamID'];?>"><button class="btn btn-sm" style="background-color: DodgerBlue;">+ Goal</button></a>
                <form method="post" style="display: inline;">
                    <button type="submit" name="action" value="teamA_minus" class="btn btn-sm" style="background-color:DodgerBlue;">- Goal</button>
                </form>
            </div>
            <?php
          ...
          ?>
            <div>
                ...
                <a href="goalscorerB.php?NAME=<?php echo $data['teamID'];?>"><button class="btn btn-sm" style="background-color: DodgerBlue;">+ Goal</button></a>
                <form method="post" style="display: inline;">
                    <button type="submit" name="action" value="teamB_minus" class="btn btn-sm" style="background-color:DodgerBlue;">- Goal</button>
                </form>
            </div>
        </div>
        <div class="actions">
            <a href="subs_out.php?NAME=<?php echo $matchID;?>"><button class="btn btn-sm" style="background-color: MediumSeaGreen;">Substitute Player</button></a>
            <form method="post" style="display: inline;">
                <button type="submit" name="action" value="end_match" class="btn btn-sm" style="background-color:Tomato;">End Match</button>
            </form>
            <a href="yellow_red.php?NAME=<?php echo $matchID;?>"><button class="btn btn-sm" style="background-color:rgb(240, 156, 1);">Yellow/Red Card</button></a>
        </div>
