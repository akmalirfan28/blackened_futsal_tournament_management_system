...
<header>
  ...
  <div class="container2">
    ...
    <?php
    $records = mysqli_query($conn, "SELECT * FROM match_schedule WHERE match_id='$match_id'");
    
    while($data = mysqli_fetch_array($records)) {
        $_SESSION['match_id'] = $match_id;
    ?>
    <form action="edit_schedule_details.php" method="POST">
        <label for="court">Court:</label>
        <select id="court" name="court" required>
            <option value="A" <?php echo ($data['court'] == 'A') ? 'selected' : ''; ?>>Court A</option>
            <option value="B" <?php echo ($data['court'] == 'B') ? 'selected' : ''; ?>>Court B</option>
        </select>

        <label for="time">Time:</label>
        <input type="time" id="time" name="time" value="<?php echo $data['match_time']; ?>" required>
