...
<header>
  ...
  <?php else: ?>
    ...
    <table>
      ...
      <tbody>
        <?php
        $records = mysqli_query($conn, "SELECT * FROM team WHERE tournament_id='$tournamentID'");
        $i = 1;
        while($data = mysqli_fetch_array($records))
        
        {
          ?>
            <tr>
                    ...
                    <td>
                    <a href="listOfPlayer.php?NAME=<?php echo $data['teamID'];?>"><button class="btn btn-sm">View</button></a>
                    </td>
        </tr>
        <?php
        $i++;
